...
export default function SearchHelloMallScreen(props) {
  const [min1, setMin1] =  useState();
  const [max1, setMax1] =  useState();
  const [noProdBool, setNoProdBool] =  useState(0);
  ...
  const { loadingHelloMall, errerrorHelloMallor, usersHelloMAll } = ListHelloSeller;


  const productListWithoutPagination = useSelector((state) => state.productListWithoutPagination);
  const { loading1 , 
    
          error1, products1 } = productListWithoutPagination;
  ...
  const getFilterUrl = (filter) => {
    ...
    return `/searchhellomall/category/${filterCategory}/name/${filterName}/min/${filterMin}/max/${filterMax}/rating/${filterRating}/order/${sortOrder}/pageNumber/${filterPage}/flash_sale/${filterflash_sale}`;
  };
  return (
    <div className='SearchMain'>
       {loading || loadingHelloMall ? (
          <LoadingBox></LoadingBox>
        ) : error || errerrorHelloMallor? (
          <MessageBox variant="danger">{error}</MessageBox>
        ) : (
          <div> <Link style={{color: "#DE3D3A"}}  className='selectDisplayNone' to={`/searchhellomall/name/`}>Remove Filter</Link></div>
      ...
      <div className="row top">
        <div className="col-1">
          ...
          <div>
            ...
            <Link style={{color: "#DE3D3A"}} to={`/searchhellomall/name/`}>Remove filter</Link>
          </div>
        </div>
            ...
            <>
              ...
              <div className="ProductRow center">
                {products1.map((product) => (
                        ...
                        product.seller == "62c1793214503d6712428ce4" ?
                        <>
