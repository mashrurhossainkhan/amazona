...
const CardsForShopTypeSelectionSellers = () =>{
    ...
    const handleShopType = (e,shoptype) =>{
        e.preventDefault();
        var ShopTypeObject = {'email':JSON.parse(retrieve_SellerReg_Page1Obj).email,'phoneNo':JSON.parse(retrieve_SellerReg_Page1Obj).phoneNo ,'shoptype': shoptype};
        ...
        return(
            <Box
                ...
                {Data_Shop_Sellect_seller.map((data)=>{
                    ...
                    <CardActions>
                    <Button variant="outlined" color="error" onClick = {(e)=>handleShopType(e,data.ShopType)}>Get Started</Button>
                    </CardActions> 
                    </Card>
                    </Box>
                    )
    ...
    const Back_to_page_one = () => {
        return(
            <div>{history.push('/marchant_registration')}</div>           
        )   
    } 
    return(
        <>
